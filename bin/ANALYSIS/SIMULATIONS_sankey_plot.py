...
'''
For publication, run the notebook SIMULATIONS_sankey_plot.ipynb and rearange the nodes in the interactive output
'''
...
def plotFlowDiagram(df, plotTitle,hide_labels):
    dummy_df = pd.DataFrame({'cn_true': [5]*100000, 'cn_alpaca': [5]*100000})
    ...
    # all_states=list(unique_cn_true_states)+list(unique_cn_predicted_states+len(unique_cn_true_states))
    ...
    predicted_labels[-1] = predicted_labels[-1]+'+'
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        node=dict(
            ...
            y = [0,0.2,0.4,0.6,0.8,0.8,0,0.16,0.33,0.5,0.66,0.73],
            color="white"
        ),
        link=dict(
            source=sources,
...
def main():
    ...
    fig = plotFlowDiagram(scores_all_tumours,'',hide_labels=True)
    fig.write_image(f'sankey_plot_publication_no_labels.pdf')
    fig = plotFlowDiagram(scores_all_tumours,'',hide_labels=False)
