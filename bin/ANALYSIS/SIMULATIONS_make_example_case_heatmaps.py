...
from python_functions import get_chr_table, clean_output, flat_list, read_tree_json, compare_alpaca_to_true, get_unique_lists,get_tree_edges
...
class plot_heatmap_with_tree_compare_with_true_solution_publication:
    def __init__(self, alpaca_output, input_data_directory, chr_table_file, wgd_clones, max_cpn_cap=99, allele='A', true_solution_df='', plot_comparison=False, sort_alleles=False, color_map=None):
        ...
        ######
        self.fig = plot_heat_map_with_true_solution(self.df.copy(), allele, self.fig, tree_graph_df, color_map, self.number_of_clones, chr_table_file=chr_table_file,plot_comparison=False)
        if plot_comparison:
        ...
        for clone in self.df.clone.unique():
            ...
            if i != len(self.df.clone.unique()) - 1:
                ...
                self.fig.update_xaxes(
                    tickmode='array',
                    tickfont=dict(size = chromosome_lables_font_size),
        ...
        annotations=[
                                   {'text': f'Clone proportions<br>in samples:', 'x': 0.93, 'y': -0.06, 'xref': 'paper', 'yref': 'paper', 'align': 'left', 'showarrow': False, 'font': {'size': annotation_font_size}},
                                   {'text': f'Seg. solved:', 'x': 1, 'y': -0.04, 'xref': 'paper', 'yref': 'paper', 'align': 'left', 'showarrow': False, 'font': {'size': annotation_font_size}},
                                   {'text': f'incorrectly', 'x': 0.99, 'y': -0.06, 'xref': 'paper', 'yref': 'paper', 'align': 'left', 'showarrow': False,'font': {'size': annotation_font_size,'color':'#DD5D46'}},
                                   {'text': f'correctly', 'x': 1, 'y': -0.1, 'xref': 'paper', 'yref': 'paper', 'align': 'left', 'showarrow': False,'font': {'size': annotation_font_size,'color':'#616CAD'}},
                                   ]
        '''if allele=='A':
        ...
        self.fig.update_layout(margin=dict(t=30, b=15, l=10, r=20),
...
def main():    
    ...
    for tumour_id in selected_tumours:
        ...
        for allele in ['A', 'B']:
            heatmap = plot_heatmap_with_tree_compare_with_true_solution_publication(alpaca_output=alpaca_output,input_data_directory=tumour_input_directory, chr_table_file=chr_table_file,wgd_clones=wgd_clones, max_cpn_cap=8, allele=allele, true_solution_df=true, plot_comparison=True, sort_alleles=False)
